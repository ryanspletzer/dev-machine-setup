...
) | Where-Object -FilterScript { Test-Path -Path $_ } | Select-Object -First 1
if (-not $settingsPath) { Write-Error -Message "Windows Terminal settings.json not found" }
...
if ($settingsPath) {
    ...
    $pwshProfile = $settings.profiles.list |
        Where-Object -FilterScript { $_.name -eq 'PowerShell' }
