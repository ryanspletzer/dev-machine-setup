#!/bin/bash
# setup.sh
# Installs prerequisites and runs the Ansible playbook for Debian
#
...
if ! command -v apt-get >/dev/null 2>&1; then
  echo "Error: apt-get not found. This script is intended for Debian systems." | tee -a "$LOG_FILE"
  exit 1
fi
...
if [ "$CI_MODE" = true ]; then
  sudo -n apt-get install -y gnupg python3 python3-pip 2>&1 | tee -a "$LOG_FILE"
else
