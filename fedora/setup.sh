#!/bin/bash
# setup.sh
# Installs prerequisites and runs the Ansible playbook for Fedora
#
...
if ! command -v dnf >/dev/null 2>&1; then
  echo "Error: dnf not found. This script is intended for Fedora systems." | tee -a "$LOG_FILE"
  exit 1
fi

# Update package lists
echo "Updating package cache..." | tee -a "$LOG_FILE"
if [ "$CI_MODE" = true ]; then
  sudo -n dnf makecache 2>&1 | tee -a "$LOG_FILE"
else
  echo "$SUDO_PASSWORD" | sudo -S dnf makecache 2>&1 | tee -a "$LOG_FILE"
fi

# Install dependencies
echo "Installing required dependencies..." | tee -a "$LOG_FILE"
if [ "$CI_MODE" = true ]; then
  sudo -n dnf install -y python3 python3-pip 2>&1 | tee -a "$LOG_FILE"
else
  echo "$SUDO_PASSWORD" | sudo -S dnf install -y python3 python3-pip 2>&1 | tee -a "$LOG_FILE"
fi

# Install Ansible
echo "Installing Ansible..." | tee -a "$LOG_FILE"
if [ "$CI_MODE" = true ]; then
  sudo -n dnf install -y ansible 2>&1 | tee -a "$LOG_FILE"
else
  echo "$SUDO_PASSWORD" | sudo -S dnf install -y ansible 2>&1 | tee -a "$LOG_FILE"
fi
